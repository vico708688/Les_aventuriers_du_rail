...
class GameApp:
    ...
    def claim_route(self, city1, city2, route):
        if not self.game.current_player.action_done:
            ...
            if self.game.current_player.remaining_trains <= 0:
                print("fin")
                self.game.end_game()
    ...
    def next_turn(self):
        for player in self.game.players:
            print(f'Joueur {player.name} a {player.remaining_trains} wagons restants.')
        self.game.train_card_draw_count = 0
        self.game.next_player()
        self.update_all()
        self.game.current_player.action_done = False
    ...
    def show_final_scores(self):
        ...
        label_title = tk.Label(top, text="Scores finaux", font=("Helvetica", 16, "bold"))
        label_title.pack(pady=10)

        # Tri facultatif des joueurs par score décroissant
        sorted_players = sorted(self.game.players, key=lambda p: p.score, reverse=True)

        for player in sorted_players:
            name = player.name
            score = player.score
            lbl = tk.Label(top, text=f"{name} : {score} points", font=("Helvetica", 12))
            lbl.pack()
